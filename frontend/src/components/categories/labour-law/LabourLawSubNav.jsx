...
import { subNav } from "../../../data/labour-law";

const EASE = [0.22, 1, 0.36, 1];

/**
 * 18-service accordion — 3 groups (Statutory Contribution / Establishment
 * & Workplace / Contract Labour & Audit) instead of Registrations'
 * 2-group split, demonstrating the pattern generalizes to a different
 * group count. Same collapsed-by-default, single-open behaviour.
 */
export const LabourLawSubNav = () => {
  const [openId, setOpenId] = useState(null);

  return (
    <section id="labour-services" data-testid="labourlaw-subnav-section" className="bg-ak-mist/40 py-20 md:py-28">
      <Container>
        ...
        <Reveal delay={0.1} className="mt-12 divide-y divide-ak-ink/[0.07] overflow-hidden rounded-2xl border border-ak-ink/[0.07] bg-white">
          {subNav.groups.map((g) => {
            ...
            return (
              <div key={g.id}>
                <button
                  ...
                  data-testid={`labourlaw-accordion-toggle-${g.id}`}
                  className="flex w-full items-center justify-between gap-4 px-6 py-6 text-left transition-colors hover:bg-ak-mist/30 md:px-8"
                >
                        ...
                        {[0, 1].map((col) => (
                          <div key={col} className="divide-y divide-ak-ink/[0.05]">
                            {g.items.filter((_, idx) => idx % 2 === col).map((item, idx) => {
                              ...
                              return (
                                <a key={item.label} href={item.href} data-testid={`labourlaw-item-${g.id}-${num}`} className="group flex items-center gap-3.5 px-6 py-3.5 transition-colors hover:bg-white md:px-8">
