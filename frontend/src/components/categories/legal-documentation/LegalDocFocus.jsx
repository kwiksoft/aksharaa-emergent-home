...
import { focusServices } from "../../../data/legal-documentation";

const OFFSET = ["", "lg:mt-8", "", "lg:mt-8", "", "lg:mt-8"];

export const LegalDocFocus = () => (
  <section data-testid="legaldoc-focus-section" className="bg-ak-mist/40 py-20 md:py-28">
    <Container>
      ...
      <RevealGroup className="mt-14 grid grid-cols-1 items-start gap-5 sm:grid-cols-2 lg:grid-cols-3 lg:gap-6" stagger={0.08}>
        {focusServices.items.map((d, i) => (
          <RevealItem key={d.num} className={OFFSET[i]}>
            <a href={d.href} data-testid={`legaldoc-focus-card-${d.num}`} className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-ak-ink/8 bg-[#FAF6EE] p-7 shadow-sm transition-all duration-500 hover:-translate-y-1.5 hover:border-ak-orange/30 hover:shadow-2xl">
