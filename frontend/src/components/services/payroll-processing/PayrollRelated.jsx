      ...
      <RevealGroup className="grid grid-cols-1 gap-5 md:grid-cols-3" stagger={0.1}>
        {related.items.map((r) => (
          <RevealItem key={r.title}>
            <a href={r.href} className="group flex h-full flex-col rounded-2xl border border-ak-ink/[0.07] bg-ak-mist/30 p-6 transition-all hover:-translate-y-1 hover:border-ak-orange/30 hover:shadow-xl">
              ...
              <span className="mt-4 inline-flex items-center gap-1.5 text-[13px] font-semibold text-ak-orange">
                Explore <Icon name="arrowRight" className="h-3.5 w-3.5 transition-transform group-hover:translate-x-1" strokeWidth={2.4} />
              </span>
            </a>
          </RevealItem>
        ))}
