...
import { faqs } from "../../../data/svc-flexi-staffing";

const EASE = [0.22, 1, 0.36, 1];

export const FlexiFaqs = () => {
  const [openIdx, setOpenIdx] = useState(0);
  return (
    <section id="svc-faqs" data-testid="flexi-faqs-section" className="bg-ak-mist/40 py-20 md:py-28">
      <Container>
        ...
        <Reveal delay={0.1} className="mt-10 divide-y divide-ak-ink/[0.08] rounded-2xl border border-ak-ink/[0.07] bg-white">
          {faqs.items.map((f, i) => {
            const isOpen = openIdx === i;
            return (
              <div key={f.q}>
                <button onClick={() => setOpenIdx(isOpen ? -1 : i)} data-testid={`flexi-faq-${i}`} className="flex w-full items-center justify-between gap-4 px-6 py-5 text-left md:px-7">
