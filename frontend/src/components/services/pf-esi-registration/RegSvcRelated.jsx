...
export const RegSvcRelated = () => (
  <section id="svc-related" data-testid="reg-svc-related-section" className="bg-ak-mist/40 py-14 md:py-20">
    <Container>
      ...
      <RevealGroup className="mt-12 grid grid-cols-1 gap-5 md:grid-cols-3" stagger={0.1}>
        {related.items.map((r) => (
          <RevealItem key={r.title}>
            <a
              href={r.href}
              data-testid={`reg-svc-related-${r.title.replace(/\s+/g, "-").toLowerCase()}`}
              className="group flex h-full flex-col rounded-2xl border border-ak-ink/[0.07] bg-white p-6 transition-all duration-300 hover:-translate-y-1 hover:border-ak-orange/30 hover:shadow-xl"
            >
              <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-ak-ink text-white transition-colors group-hover:bg-ak-orange">
